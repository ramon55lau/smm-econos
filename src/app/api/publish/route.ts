...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { adId, destinations, socialAccountId, agent } = body;
          ...
          if (dlRes.ok) {
            ...
            if (!isVideo) {
              try {
                ...
                // --- AGENT INFO CARD COMPOSER OVERLAY ---
                if (agent) {
                  try {
                    console.log(`[SMM Agent Overlay] Componiendo la tarjeta del asesor: ${agent.name}`);
                    const agentPhotoUrl = agent.photo || "https://mhestate.es/assets/img/client-placeholder.jpg";
                    const avatarRes = await fetch(agentPhotoUrl, { signal: AbortSignal.timeout(6000) });
                    const avatarBuf = avatarRes.ok
                      ? Buffer.from(await avatarRes.arrayBuffer())
                      : await sharp({ create: { width: 100, height: 100, channels: 4, background: { r: 176, g: 141, b: 109, alpha: 1 } } }).png().toBuffer();

                    const circleMask = Buffer.from(
                      `<svg width="100" height="100"><circle cx="50" cy="50" r="50" fill="white" /></svg>`
                    );
                    const agentCircle = await sharp(avatarBuf)
                      .resize(100, 100)
                    ...
                    const cardLeft = Math.max(0, size - 236 - 20);
                    const cardTop = Math.max(0, size - 164 - 20);

                    finalBuffer = await sharp(finalBuffer)
                      .composite([{ input: agentCard, top: cardTop, left: cardLeft }])
                      .jpeg({ quality: 90 })
                      .toBuffer();

                    console.log(`[SMM Agent Overlay] Composición de tarjeta del asesor completada con éxito.`);
                  } catch (overlayErr: any) {
                    console.error("[SMM Agent Overlay] Error during composition:", overlayErr.message);
                  }
                }
              } catch (sharpErr: any) {
